    ...
    <form action={action} className="flex flex-col gap-3 rounded-2xl border border-border bg-surface p-4">
          ...
          {category ? (
            <>
              ...
              <span className="ms-2 text-xs font-normal text-muted">
                {category.productsCount} {dict.productsCount}
              </span>
            </>
          ) : (
            dict.add
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        ...
        <label className="flex flex-col gap-1 sm:col-span-2">
          <span className="text-xs font-semibold text-muted">{dict.gradient}</span>
          <input name="gradient" defaultValue={category?.gradient ?? "from-violet-500 to-fuchsia-500"} required className={FIELD} />
        </label>
        <label className="flex flex-col gap-1">
